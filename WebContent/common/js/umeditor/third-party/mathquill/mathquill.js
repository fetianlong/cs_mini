...
var P = (function(prototype, ownProperty, undefined) {
  ...
  function P(_superclass /* = Object */, definition) {
    ...
    // context.
    //
    //  : the Chrome inspector shows all created objects as `C`
...
function createRoot(jQ, root, textbox, editable) {
  ...
  var textareaManager = manageTextarea(textarea, {
    ...
    paste: function(text) {
      //  HACK the parser in RootTextBlock needs to be moved to
...
var RootTextBlock = P(MathBlock, function(_) {
  _.renderLatex = function(latex) {
    ...
    var mathMode = string('$').then(latexMathParser)
      ...
      .skip(string('$').or(eof))
      .map(function(block) {
        // HACK : this shouldn't have to have access to cursor
...
LatexCmds.textmd = P(MathCommand, function(_, _super) {
  ...
  _.createBlocks = function() {
    //: another possible Law of Demeter violation, but this seems much cleaner, like it was supposed to be done this way
    this.ends[L] =
    this.ends[R] =
      InnerTextBlock();
  ...
  _.finalizeInsert = function() {
    // HACK blur removes the TextBlock
...
var InnerTextBlock = P(MathBlock, function(_, _super) {
  ...
  _.write = function(cursor, ch, replacedFragment) {
    ...
    if (ch !== '$') {
      ...
      //  HACK: pretend no prev so they don't get merged when
  ...
  _.focus = function() {
    ...
    if (textblock[R].ctrlSeq === textblock.ctrlSeq) { //: seems like there should be a better way to move MathElements around
      var innerblock = this,
    ...
    if (this._replacedFragment) {
      ...
      this.jQ =
        this._replacedFragment.jQ.addClass('blur').bind(
          'mousedown mousemove', //: is monkey-patching the mousedown and mousemove handlers the right way to do this?
          function(e) {
...
LatexCmds.editable = P(RootMathCommand, function(_, _super) {
  ...
  _.jQadd = function() {
    var self = this;
    // : this entire method is a giant hack to get around
