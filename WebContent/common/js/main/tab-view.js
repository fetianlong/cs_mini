	...
	var tabView_maxNumberOfTabs = 20;	// Maximum number of tabs
	...
	var tabDivCounter = 0;
	var closeImageHeight = 10;	// Pixel height of close buttons
	var closeImageWidth = 10;	// Pixel height of close buttons
	var tabDiv;
	
	function setPadding(obj,padding){
		var span = obj.getElementsByTagName('SPAN')[0];
		span.style.paddingLeft = 2 + 'px';	
		span.style.paddingRight = 2 + 'px';	
	...
	function showTab(parentId,tabIndex)
	{
		...
		var aTab = document.getElementById("TabDiv");//parentObj.getElementsByTagName('DIV')[0];
		countObjects = 0;
		var startPos = 2;
	...
	function tabClick()
	{
		...
		showTab(this.parentNode.parentNode.parentNode.id,idArray[idArray.length-1].replace(/[^0-9]/gi,''));
...
//			dhtmlgoodies_tabObj[mainContainerID].style.width = width;
			...
			var tabDivContent = document.createElement('DIV');		
			tabDivContent.id = "TabContentDiv";
			tabDivContent.className = 'dhtmlgoodies_tabPane_content';	
			...
			tabDivContent.appendChild(tabDiv);
			dhtmlgoodies_tabObj[mainContainerID].appendChild(tabDivContent);
			tabDiv.className = 'dhtmlgoodies_tabPane';			
			tabView_countTabs[mainContainerID] = 0;

		}else{
			tabDiv =  document.getElementById("TabDiv");// dhtmlgoodies_tabObj[mainContainerID].getElementsByTagName('DIV')[0]getElementsByTagName('DIV')[0];
		...
		for(var no=0;no<tabTitles.length;no++){
			...
			if(closeButtonArray[no]){
				...
				closeButton.src = 'common/css/images/close.gif';
...
//			aTab.appendChild(img);
	...
	function resetActiveTabIndex(parentId)
	{
		var tabTitleCounter = 0;
		var divs = dhtmlgoodies_tabObj[parentId].getElementsByTagName('DIV');
		...
		for(var no=0;no<divs.length;no++){
			if(divs[no].id.indexOf('tabTab')>=0){
				if(divs[no].className=='tabActive'){
					activeTabIndex[parentId]=tabTitleCounter;
					return;
				}
				tabTitleCounter++;
			}
	...
	{
		//if(tabView_countTabs[parentId]>=tabView_maxNumberOfTabs){alert("最多同时打开" + tabView_maxNumberOfTabs + "个窗口，请关闭其它窗口后再打开！");return;}	// Maximum number of tabs reached - return
		var div = document.createElement('DIV');
		div.className = 'dhtmlgoodies_aTab';
		div.style["height"] = (document.body.offsetHeight - 200)+"px";
		if(!dhtmlgoodies_tabObj[parentId]){
			initTabs(parentId, new Array(), 0, '', '', new Array());
		}
		dhtmlgoodies_tabObj[parentId].appendChild(div);		

		var tabId = initTabs(parentId,Array(tabTitle),0,'','',Array(closeButton),true);
		if(tabContent)div.innerHTML = tabContent;
		else if(tabContentUrl){		
			...
			div.innerHTML = "<iframe class=\"ifra\" width=\"100%\" height=\"100%\" src=\"" + tabContentUrl + "\">";
		}
	}
		...
		if(tabLabel){
			...
			if(document.getElementById('tabTab' + parentId + '_' + tabIndex)){
				...
				if(activeTabIndex[parentId] == tabIndex){
					...
					resetActiveTabIndex(parentId);
				}
			}			
		}
		var tabContentDiv = document.getElementById('TabContentDiv');
		if(Number(tabContentDiv.scrollWidth) - Number($('#TabContentDiv').width()) < 2){
			$('#leftButn').hide();
			$('#rightButn').hide();
		}
	}
...
function moveLeft(){
	document.getElementById("TabContentDiv").scrollLeft = document.getElementById("TabContentDiv").scrollLeft - 200;
}
function moveRight(){
	document.getElementById("TabContentDiv").scrollLeft = document.getElementById("TabContentDiv").scrollLeft + 200;	
}
...
var current;
var toLeft=function(){
	$("#TabContentDiv").scrollLeft($("#TabContentDiv").scrollLeft()-speed);
};
...
$(function(){
	$("#leftButn").mousedown(function(){
		stopScroll();
		timer=setInterval(toLeft,50);	
	});
	$("#rightButn").mousedown(function(){	
		stopScroll();
		timer=setInterval(toRight,50);
	});
	$("#leftButn,#rightButn").mouseup(stopScroll);
	$("#leftButn,#rightButn").mouseleave(stopScroll);
});