var BMapLib = window.BMapLib = BMapLib || {}; (function() {
    ...
    (function(n, I) {
        ...
        D = (function() {
            var L = "t,webkitT,MozT,msT,OT".split(","),
            K,
            J = 0,
            m = L.length;
            for (; J < m; J++) {
        ...
        H = D ? "-" + D.toLowerCase() + "-": "",
        q = w("transform"),
        B = w("transitionProperty"),
        p = w("transitionDuration"),
        s = w("transformOrigin"),
        F = w("transitionTimingFunction"),
        i = w("transitionDelay"),
        E = (/android/gi).test(navigator.appVersion),
        l = (/iphone|ipad/gi).test(navigator.appVersion),
        v = (/hp-tablet/gi).test(navigator.appVersion),
        o = w("perspective") in r,
        C = "ontouchstart" in n && !v,
        h = D !== false,
        j = w("transition") in r,
        k = "onorientationchange" in n ? "orientationchange": "resize",
        f = C ? "touchstart": "mousedown",
        x = C ? "touchmove": "mousemove",
        g = C ? "touchend": "mouseup",
        A = C ? "touchcancel": "mouseup",
        e = (function() {
            if (D === false) {
                return false
            }
        ...
        z.prototype = {
            ...
            _move: function(Q) {
                ...
                if (O.options.zoom && C && Q.touches.length > 1) {
                    ...
                    m = this.originX - this.originX * O.lastScale + this.x,
                    S = this.originY - this.originY * O.lastScale + this.y;
            ...
            _end: function(Q) {
                ...
                N = (Q.timeStamp || Date.now()) - O.startTime,
                S = O.x,
                P = O.y,
                U,
                T,
            ...
            _resetPos: function(K) {
                var m = this,
                L = m.x >= 0 ? 0 : m.x < m.maxScrollX ? m.maxScrollX: m.x,
                J = m.y >= m.minScrollY || m.maxScrollY > 0 ? m.minScrollY: m.y < m.maxScrollY ? m.maxScrollY: m.y;
                if (L == m.x && J == m.y) {
                    if (m.moved) {
                        m.moved = false;
            ...
            _bind: function(K, J, m) { (J || this.scroller).addEventListener(K, this, !!m)
            },
            _unbind: function(K, J, m) { (J || this.scroller).removeEventListener(K, this, !!m)
            },
            destroy: function() {
                var m = this;
                m.scroller.style[q] = "";
    ...
    a.prototype = {
        ...
        _bind: function() {
            var e = "click",
            f = this;
            c.on(this.dom.nSearchBtn, e,
            function(g) {
                b(g);
                f.localSearchAction()
            });
            c.on(this.dom.changeBtn, e,
            function(g) {
                b(g);
                f.changeStartAndEnd()
            });
        ...
        transitRouteAction: function() {
            this.reset();
            var f = this.dom.startText.value,
            e = this.dom.endText.value;
        ...
        _initCityTab: function() {
            ...
            var h = "数据1|131,数据2|289,数据3|332,数据4|132,数据5|23,数据6|16,数据7|6,数据8|7,数据9|17,数据10|24,数据11|21,数据12|25,数据132,数据14|30,数据15|15,数据16|26,数据17|18,数据18|31,数据19|9,数据20|19,数据21|22,数据22|20,数据23|11,数据24|8,数据25|10,数据26|27,数据27|32,数据28|12,数据29|13,数据30|28,数据31|29".split(",");
            var m = [];
            ...
            var g = '<div style="height:40px;line-height:40px;background:#8C8C8C;color:#fff;padding-left:10px;font-size:12px;"><span style="float:left;">璇烽€夋嫨鍏蜂綋鍩庡競</span><span style="float:right;padding:0 10px;" onclick="this.parentNode.parentNode.style.display=\'none\'">数据测试1</span></div><div style="overflow:hidden;width:100%;height:200px;position:relative;" id="BMapLib_scroll"><ul id = "BMapLib_cityList" style="list-style:none;padding:0px;margin:0px;">';
            for (var f = 0,
            e = m.length; f < e; f++) {
        ...
        selectCityCallback: function(r) {
            ...
            var e = r.content.sub,
            g = r.content.area_code,
            o = r.content.area_name;
            ...
            if (!this.cityListSub[g]) {
                ...
                var p = /数据1|数据2|数据3|数据4/g;
                if (o.match(p)) {
                    n += "<li style='padding-left:30px;height:30px;line-height:30px;font-size:14px;border-bottom:1px solid #ccc;background:#f3f3f3;' type='3' cityname='" + o + "'>鍏ㄥ競</li>"
                }
                for (var m = 0,
                f = e.length; m < f; m++) {
