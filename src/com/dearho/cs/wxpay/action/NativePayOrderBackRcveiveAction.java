...
public class NativePayOrderBackRcveiveAction extends AbstractAction{
	
	private static final long serialVersionUID = 4598081722908212830L;
	
	public static final Log logger = LogFactory.getLog(NativePayOrderBackRcveiveAction.class);
	...
	public void handle() throws Exception {
		...
		try {
			logger.error("NativePayOrderBackRcveiveAction接收后台通知开始");
          ...
          //  XmlStream.toXML(object)
			...
			if(order!=null && Consts.SUCCESS.equalsIgnoreCase(order.getReturnCode())){
				...
				if(Consts.SUCCESS.equalsIgnoreCase(order.getResultCode())){
					...
					orderPayService.orderPayOnlineCallBack(payResponseMessage, OrderPayService.STEP_CRATE);
			...
			logger.error("NativePayOrderBackRcveiveAction接收后台通知结束");
		}  catch (Exception e) {
			e.printStackTrace();
			logger.error(e);
			XmlResult xmlResult=new XmlResult("SUCCESS", "OK");
			...
			if(order!=null){
				...
				interfaceMessageLog.setRequestMessage("("+resultXml+")=");
			}
