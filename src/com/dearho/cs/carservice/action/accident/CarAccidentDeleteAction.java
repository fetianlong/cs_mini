 ...
 */
public class CarAccidentDeleteAction extends AbstractAction{
	...
	public String process() {
		try {
			...
			for (int i = 0; i < checkdel.length; i++) {
				CarAccident data = carAccidentService.searchCarAccidentById(checkdel[i]);
				contentMap.put("编号", data.getCode());
			}
			SystemOperateLogUtil.sysDeleteOperateLog(user, "车辆事故管理", contentMap);
			
			carAccidentService.deleteCarAccident(checkdel);
